...
class DataCollectorGUI:
    def __init__(self, save_path):
        ...
        for i in range(num_peaks):
            peak_x = i * peak_spacing + 2 # 山の頂点のX座標
            self.monitoring_points_x.append(peak_x)
            if i < num_peaks -1:
                valley_x = i * peak_spacing + 5 # 谷の底のX座標
                self.monitoring_points_x.append(valley_x)
        # これで、[2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32] の11点が監視対象になる
    ...
    def reset_game(self):
        # ▼▼▼ リセット時に-1を保存するロジックを追加 ▼▼▼
        if hasattr(self, 'placed_blocks') and self.placed_blocks:
            # 現在の盤面状態を取得
            state_before = self.board_state.copy()
            # 配置済みのブロックがある場合のみ、リセット操作を記録
            # placed_x = -1 はリセット操作を示す特別な値
            self.save_data(state_before, self.current_block_type, -1, -1)
            print("データ保存: リセット操作を-1として記録しました。")
    ...
    def save_data(self, state, block_type, x, rot):
        ...
        if len(state) != len(self.monitoring_points_x):
            ...
            return
        row = state + [block_type, int((x - 2)/3), rot]
