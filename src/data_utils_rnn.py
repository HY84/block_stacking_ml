...
class StackingDataset(Dataset):
    ...
    def __len__(self):
        # シーケンスを作成できる分だけがデータセットの長さになる
        return len(self.X) - SEQUENCE_LENGTH + 1


    def __getitem__(self, idx):
        # idxからSEQUENCE_LENGTH分のデータをスライスしてシーケンスを作成
        sequence = self.X[idx : idx + SEQUENCE_LENGTH]
        
        # ラベルは、シーケンスの最後の時点での行動
        label_slot = self.y_slot[idx + SEQUENCE_LENGTH - 1]
        label_rotation = self.y_rotation[idx + SEQUENCE_LENGTH - 1]
        # シーケンスとラベルをタプルとして返す
        return sequence, (label_slot, label_rotation)
